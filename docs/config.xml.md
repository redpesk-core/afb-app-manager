...
The packages have a structure similar to the structure of the widget as described
by the W3C's technical recommendations [Packaged Web Apps (Widgets)][widgets]
and [XML Digital Signatures for Widgets][widgets-digsig]
...
The file MUST be at the root of the package and MUST be case sensitively name
***config.xml***.
...
For example, a publisher could provide a package containing a service,
...
the ability to use bindings exported by other packages.
...
can provide more than one application in the same package.
...
of the main of the package, has the name "main".
The value given here must be unique within the widget file.
It will be used in other places of the package config.xml file to
designate the unit.
